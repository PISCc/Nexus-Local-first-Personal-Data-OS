...
当前处于：`M0–M5` 巩固完成，M5 评估已通过；M6 尚未开始。

### M0–M5 巩固记录

- 已重新验证 M0–M2 的工程、扫描、解析、资源上限和安全错误边界。
- 初始正文索引改为按有界批次暂存正文，并和同批元数据、FTS 一起提交；取消不会留下
  孤立正文，确认删除的文件会清理旧正文。
- 已补充 M3 控制字符查询拒绝、M4 增量生命周期和 M5 异常向量回退、取消前模型状态、路径
  刷新批次等回归验证。
- 验收明细见 `docs/acceptance/0022-m0-m5-hardening.md`。
...
- 已明确 Unix epoch 毫秒时间戳、已知扩展名的稳定类型标签、不默认计算哈希以及不跟随符号链接的策略。
...
- 监听任务复用 `RescanControl`，并使用关闭标记停止接收新事件；停止时等待线程退出，
  让未提交批次继续受 M4.2 的事务边界保护。手动重扫期间暂停既有 watcher 但继续消费
  事件，变化会合并为一次完整追赶重扫；失败或取消会恢复原 watcher，切换目录时新
  watcher 建立后会先做边界校准。
