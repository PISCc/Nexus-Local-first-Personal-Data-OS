...
- `file_metadata` 保存路径、文件名、扩展名、大小、可选的三个 Unix epoch 毫秒时间戳和可选类型标签；核心扫描器对已知扩展名使用稳定的 MIME-like 映射，未知类型保持为空。
...
任务同时使用已有的 `RescanControl`、独立关闭标记和暂停标记。手动重扫开始前暂停旧
监听，但继续消费事件；暂停期间的任意变化会合并为一次完整追赶重扫，重扫失败或取消
后恢复原监听。成功切换监听目录时，新监听先建立事件源再做一次边界校准；应用退出时
先取消重扫，再取消监听并等待线程结束。批次提交失败会保留待处理批次并延迟重试，
取消后不再接收新事件。
