...
- 当前 Git 仓库还没有 commit，也没有远程仓库配置。

### 当前阶段

当前处于：`Research / Plan`。

尚未进入实现阶段。进入 M0 前，必须先确认仓库根目录、包管理器、SQLite Rust 驱动和测试范围等架构决定。

### 初始同步状态

- 本次同步将把当前规划文档提交到 GitHub 远程仓库。
...
在开始任何代码实现前，先由项目负责人确认：

1. 正式仓库根目录。
2. 是否采用 pnpm workspace。
3. 是否采用 Tauri 2。
4. 是否采用 `rusqlite + bundled`。
5. M0 是否暂缓 Playwright。
6. CI 的目标平台。
7. M0 的日志和数据库失败行为。
8. 是否只创建 `nexus-core` 与 `nexus-db`。
